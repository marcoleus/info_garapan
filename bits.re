...
if($ic){L(5);$s=base_run($r["verify"],http_build_query([str_replace('" id="token','',$r["token"][1][0])=>$r["token"][2][0],$r["token"][1][1]=>$r["token"][2][1],"antibotlinks"=>implode(" ",$r["antb"][2]),"captcha"=>"mooncaptchav2","answer"=>$ic]))["notif"];
...
if($coi==null){goto wd;}
$r2=base_run(host."withdraw/withdraw",http_build_query([$r1["token"][1][0]=>$r1["token"][2][0],"amount"=>$r1["amount"],"method"=>$r1["code"][$coi-1],"wallet"=>$wallet]))["notif"];
if($s[1][2]=="Good job!"){print h.$s[1][2];r();print h.$s[1][3].n;line();goto menu;}else{print m.$s[1][2].n;L(5);goto wd;}
...
//print_r($u_acv);
//die(print_r($amn));
return ["res"=>$r,"cloudflare"=>$cloudflare[0],"firewal"=>$firewal[0],"username"=>$username[1],"balance"=>$balance[1][0],"token"=>$t_cs,"time"=>$tmr[4],"notif"=>[$n_c[1],$n_r],"antb"=>antb($ab),"verify"=>$verify[0],"view"=>$view[1],
