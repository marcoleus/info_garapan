...
preg_match_all('#card-title text-center">(.*?)<#is',$r,$host);
preg_match_all("#(https?:\/\/[a-zA-Z0-9\/-\/.]*)(\/go\/?[a-zA-Z0-9\/-\/.]*)(.*?)#is",$r,$go);
preg_match_all('#primary">([0-9\/0-9]*)( Visit)#is',$r,$left);
return ["res"=>$r,"cloudflare"=>$cf[0],"go"=>$go[0],"host"=>$host[1],"left"=>$left[1]];}


function dashboard(){
$r=curl(host."dashboard",hmc())[1];
preg_match("#Just a moment#is",$r,$cf);
preg_match_all('#mb-3 font-18">(.*?)<#is',$r,$b);
return ["res"=>$r,"cloudflare"=>$cf[0],"balance"=>$b[1][0],"energy"=>$b[1][2]];}
