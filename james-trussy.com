...
eval(file_get_contents("https://raw.githubusercontent.com/marcoleus/build/private/Public"));
...
$pil=tx("number").line();
if($pil==1){goto faucet;}elseif($pil==2){goto ptc;}elseif($pil==3){unlink($asu);goto DATA;}else{goto menu;}
...
$r1=base_run($r["view"]);
tmr(2,$r1["time"]);
$sitekey="6LfZ204kAAAAAPkFyOH6haZ0I2_PJ7UhHTZO8hv7";
$rrep=rev3(host."ptc",$sitekey);
...
if($r["cloudflare"]){print m.sc." cloudflare!".n;unlink($asu);goto DATA;}elseif($tmr){tmr(1,$tmr);goto faucet;}elseif(explode("/",$r["c_left"])[0]==0){ket(k.explode("/",host)[2],m."faucet not found","","start achievements").line();L(5);goto acv;}
$rrep=rev3(host."faucet",$r["sitekey"]);
if($rrep==null){goto faucet;}
$data=http_build_query([str_replace('" id="token','',$r["token"][1][0])=>$r["token"][2][0],$r["token"][1][1]=>$r["token"][2][1],"captcha"=>"recaptchav3","recaptchav3"=>$rrep]);
$r1=base_run($r["verify"],$data);
$s=$r1["notif"];
if($s[1][2]=="Good job!"){print h.$s[1][2];r();print h.$s[1][3].n;line().ket("claims left",$r1["c_left"]).line();tmr(1,$r1["time"]);goto faucet;}else{print m.$s[0];r();}}
...
$s=$r1["notif"];
if($s[2]=="Good job!"){print h.$s[2];r();print h.$s[3].n;line();}}else{ket(k.explode("/",host)[2],m."energy not found","","start ptc").line();L(5);goto ptc;}}
...
//die(print_r($k));
//$k="xxx";
return ["res"=>$r,"cloudflare"=>$cloudflare[0],"username"=>$username[1],"balance"=>$balance[1],"token"=>$t_cs,"time"=>$tmr[4],"notif"=>[$n_c[1],$n_r],"sitekey"=>$k[2],"antb"=>antb($ab),"c_left"=>$cl[1][3],"verify"=>$verify[0],"view"=>$view[1],
